...
var ssort2ins = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof ssort2ins, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 8', opts, function test( t ) {
	t.strictEqual( ssort2ins.length, 8, 'has expected arity' );
	t.end();
});
...
tape( 'the function sorts a strided array (increasing order)', opts, function test( t ) {
	...
	for ( i = 0; i < expectedX.length; i++ ) {
		if ( isnanf( expectedX[ i ] ) ) {
			t.strictEqual( isnanf( x[ i ] ), true, 'returns expected value. index: '+i+'. actual: '+x[i]+'. expected: '+expectedX[i]+'.' );
		} else if ( isNegativeZerof( expectedX[ i ] ) ) {
			t.strictEqual( isNegativeZerof( x[ i ] ), true, 'returns expected value. index: '+i+'. actual: '+num2str( x[i] )+'. expected: '+num2str( expectedX[i] )+'.' );
		} else if ( isPositiveZerof( expectedX[ i ] ) ) {
			t.strictEqual( isPositiveZerof( x[ i ] ), true, 'returns expected value. index: '+i+'. actual: '+num2str( x[i] )+'. expected: '+expectedX[i]+'.' );
		} else {
...
tape( 'the function returns a reference to the first input array', opts, function test( t ) {
	...
	y = new Float32Array( [ 0.0, 1.0, 2.0, 3.0, 4.0 ] );
	out = ssort2ins( x.length, 1.0, x, 1, 0, y, 1, 0 );
...
tape( 'if `order` equals `0`, the function leaves `x` and `y` unchanged', opts, function test( t ) {
	...
	ssort2ins( x.length, 0.0, x, 1, 0, y, 1, 0 );
	t.deepEqual( x, expectedX, 'returns expected value' );
	t.deepEqual( y, expectedY, 'returns expected value' );
...
tape( 'the function supports specifying negative strides (increasing order)', opts, function test( t ) {
	...
	ssort2ins( 3, 1.0, x, -2, x.length-1, y, -2, y.length-1 );
	t.deepEqual( x, expectedX, 'returns expected value' );
	t.deepEqual( y, expectedY, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports complex access patterns (increasing order)', opts, function test( t ) {
	...
	ssort2ins( 3, 1.0, x, -2, x.length-1, y, 1, 0 );
	t.deepEqual( x, expectedX, 'returns expected value' );
	t.deepEqual( y, expectedY, 'returns expected value' );
	t.end();
});
